...
body {
  /*margin: 2rem;*/
  font-family: "Raleway", Arial, Helvetica, sans-serif; /*'Open Sans', sans-serif*/
...
<main class="l-main">
    <div class="subpage">
      ...
      <h2>Answer the following questions.</h2>
          ...
          <form class="form-prevent-multiple-submits" method="POST" action="{{route('sif.followup', $followupQuestion->id)}}">
            @csrf
            <input type="hidden" name="question_id" value="{{ $followupQuestion->question_id }}">
            <input type="hidden" name="followup_id" value="{{ $followupQuestion->id }}">
              ...
              <div class="form-con">
                <div class="form-input">
                  <label class="form-lbl" for="username">
                    <span>{{$number}}.1 {{ $followupQuestion->question }}</span>
                  </label>
                  
                  @if ($followupQuestion->code_id == '1')
                    @foreach ($followupQuestion->questionChoices as $key => $choice)
                      @if($choice->label == 'Yes')
                  ...
                  @elseif ($followupQuestion->code_id == '3')
                    <div class="form-input-box">
                      <input type="text" id="answer" name="answer" placeholder="{{ $followupQuestion->question }}"/>
                    </div>
                    <br>
