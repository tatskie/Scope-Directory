	...
	<div class="col-md-12">
      ...
      <div class="alert alert-danger" role="alert" v-if="!information || information.length">
          ...
          <table cellspacing="0" cellpadding="0">
            ...
            <p style="color: #00a0e3; margin-left:10px" v-if="tif.title">PIF: {{ tif.title }}</p>
					<br>
...
<script>
  ...
  import PublishPublicProfile from '../notification/PublishPublicProfile'
    ...
    export default {
      ...
      methods: {
        loadProfile() {
            axios.get('/api/teacher/profile').then(({data}) => (this.user = data, this.card = data.card, this.scope = data.scope));
        },
