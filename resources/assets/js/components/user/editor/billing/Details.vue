...
<script>
  import '../../../common/table/Table';
    ...
    export default {
      ...
      methods: {
        loadReceipts() {
            axios.get('/api/editor/receipts').then(({data}) => (this.receipts = data));
        },

        downloadReceipt(id) {
        	// axios.get('/api/editor/download-receipt');
        	axios({
			  url: '/api/editor/download-receipt/' + id,
			  method: 'GET',
