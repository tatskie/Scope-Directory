  ...
  import EditVideo from '../videos/Edit'
    ...
    export default {
      ...
      methods: {
        ...
        loadProfile() {
            this.loading = true
            axios.get('/api/editor/profile').then(({data}) => (this.user = data, this.scope = data.scope, this.card = data.card));
            this.loading = false
        },
        ...
        loadBackData() {
            axios.get('/api/editor/back-card-data').then(({data}) => (this.back = data));
        }
      }
    }
</script>
