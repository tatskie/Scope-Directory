      ...
      <template slot="body">
        ...
        <div class="form" v-else>
            <div class="form-item" style="text-align: center;" v-show="!isPaid">
              <h1>Title: My Special Title {{ user.name }}</h1>
              <h1>PIF: My PIF Title</h1><br>
              <h1>Total Amount: $87</h1>
              <br>
...
<script>
  ...
  // import PayPal from 'vue-paypal-checkout';
    ...
    export default {
      ...
      methods: {
        loadProfile() {
            axios.get('/api/editor/profile').then(({data}) => (
              this.user = data,
        ...
        loadShippingAddress() {
            axios.get('/api/editor/shipping-address').then(({data}) => (
              this.shipping = data
              ));
        },
        ...
        myBilling() {
          ...
          this.$router.push('/editor/billing/').catch(err => {});
        },  
        ...
        paymentReceipt() {
          this.form.post('/api/editor/receipt')
            .then(() => {
        ...
        createShippingAddress() {
          this.shippingForm.post('/api/editor/shipping-address')
            .then(()=>{
              Toast.fire({
                  type: 'success',
