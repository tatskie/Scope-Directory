...
class AIFController extends Controller
{
    ...
    public function pifQuestion(Request $request)
    {
        ...
        return redirect()->route('aif.question', $question->url);
    }
    ...
    public function show(Question $question)
    {
        ...
        if($hasAnswer)
        {
            foreach ($question->followupQuestion as $followupQuestion) {
                ...
                if (!$hasFollowupAnswer) {
                    return redirect()->route('aif.followup', $followupQuestion->url);
                }
            }
            ...
            return redirect()->route('aif.question', $nextQuestion->url); // return if user has answer
        }
        ...
        return view('academia.aif.show', compact(['question', 'pages', 'questions', 'lastQuestion', 'number']));
    }
    ...
    public function followupQuestion(FollowupQuestion $followupQuestion)
    {
        ...
        if ($user->answerScore->is_done_tif == true) {
            return redirect()->intended('/academia-assessment');
        }
        ...
        return view('academia.aif.followup', compact(['followupQuestion', 'pages', 'questions', 'lastQuestion', 'number']));
    }
}
